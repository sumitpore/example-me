...
if ( ! class_exists( __NAMESPACE__ . '\\' . 'Router' ) ) {
	 ...
	 * @since      1.0.0
	 * @package    Example_Me
	 * @subpackage Example_Me/controllers
	 */
	class Router {
		...
		public function late_frontend_route_types() {
			return apply_filters(
				'example_me_late_frontend_route_types', [
		...
		public function register_route_of_type( $type ) {
			if ( in_array( $type, $this->late_frontend_route_types() ) && did_action( 'wp' ) ) {
				trigger_error( __( 'Late Routes can not be registered after `wp` hook is triggered. Register your route before `wp` hook is triggered.', Example_Me::PLUGIN_ID ), E_USER_ERROR ); // @codingStandardsIgnoreLine.
			}

			if ( in_array( $type, $this->generic_route_types() ) && did_action( 'init' ) ) {
				trigger_error( __( 'Non-Late Routes can not be registered after `init` hook is triggered. Register your route before `init` hook is triggered.', Example_Me::PLUGIN_ID ), E_USER_ERROR ); // @codingStandardsIgnoreLine.
			}
