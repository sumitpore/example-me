...
if ( ! trait_exists( 'Base_Registry' ) ) {
	...
	trait Base_Registry {
		...
		public static function set( $key, $value ) {
			if ( ! is_string( $key ) ) {
				trigger_error( __( 'Key passed to `set` method must be key', Example_Me::PLUGIN_ID ), E_USER_ERROR ); // @codingStandardsIgnoreLine.
			}
		...
		public static function get( $key ) {
			if ( ! is_string( $key ) ) {
				trigger_error( __( 'Key passed to `get` method must be key', Example_Me::PLUGIN_ID ), E_USER_ERROR ); // @codingStandardsIgnoreLine.
			}
