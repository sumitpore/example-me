...
if ( ! class_exists( 'Requirements_Checker' ) ) {
	...
	class Requirements_Checker {
		 ...
		 *
		 * @param string $example_me Name of the plugin.
		 * @param string $plugin_slug Slug of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_plugin_active( $example_me, $plugin_slug ) {
			...
			$this->add_error_notice(
				$example_me . ' is a required plugin.',
				$example_me . ' needs to be installed & activated.'
			);
		 ...
		 *
		 * @param string $example_me Example Me.
		 * @param string $plugin_slug Plugin Slug.
		 * @param string $min_plugin_version Minimum version required of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_required_plugin_version_active( $example_me, $plugin_slug, $min_plugin_version ) {
			...
			$this->add_error_notice(
				"{$example_me} {$min_plugin_version}+ is required.",
				"{$example_me} {$installed_plugin_version} is installed."
			);
		...
		private function are_required_plugins_dependency_met() {
			...
			$installed_plugins = array_filter(
				...
				function( $required_plugin_data, $required_example_me ) {
					return $this->is_plugin_active( $required_example_me, $required_plugin_data['plugin_slug'] );
				},
			...
			$plugins_installed_with_required_version = array_filter(
				$installed_plugins,
				function( $required_plugin_data, $required_example_me ) {
					return $this->is_required_plugin_version_active( $required_example_me, $required_plugin_data['plugin_slug'], $required_plugin_data['min_plugin_version'] );
				},
				ARRAY_FILTER_USE_BOTH
			);
